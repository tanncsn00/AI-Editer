"""TTS template for reup-comedy-dub skill.

Copy to projects/<slug>/run_tts.py, edit voice/speed, run.
"""
...
SPEED = 1.0                        # bump up if voice longer than clip
...
print(f"Requesting TTS: {VOICE} @ {SPEED}x ({len(text)} chars)")
...
if not audio_link and req_id:
    print(f"Polling request_id={req_id}...")
    for i in range(60):
        time.sleep(2)
...
if not audio_link:
    print("No audio link after polling")
    sys.exit(4)

ar = requests.get(audio_link, timeout=120)
OUT.write_bytes(ar.content)
print(f"Saved: {OUT} ({len(ar.content)} bytes)")
