...
import beatsData from "./top_7_dao_huu_beats.json";
...
export const Top7DaoHuu: React.FC = () => {
  return (
    <AbsoluteFill style={{ background: "#0A0612" }}>
      <Audio src={staticFile("top_7_dao_huu_nguy_hiem/voice.mp3")} />
      {beats.map((b) => (
        <Sequence key={b.index} from={Math.round(b.start * FPS)} durationInFrames={Math.round(b.duration * FPS)}>
          <SlideBg src={`top_7_dao_huu_nguy_hiem/slide_${b.index}.png`} duration={b.duration} />
        </Sequence>
      ))}
    </AbsoluteFill>
  );
