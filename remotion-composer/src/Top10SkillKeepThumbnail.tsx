...
export const Top10SkillKeepThumbnail: React.FC = () => {
  return (
    <AbsoluteFill style={{ background: BG_DEEP }}>
      <svg width={W} height={H} style={{ position: "absolute", inset: 0 }}>
        ...
        <g transform={`translate(${W / 2}, 230)`}>
          <rect x={-310} y={-44} width={620} height={88} rx={44} fill={BG_SURFACE} stroke={GREEN} strokeWidth={3} />
          <text x={0} y={14} fontSize={32} fill={GREEN} textAnchor="middle" fontFamily="'Be Vietnam Pro', sans-serif" fontWeight={800} letterSpacing="3">
            ⚡ AI ERA · EDGE NGƯỜI
          </text>
        </g>
        ...
        <g transform={`translate(${W / 2}, 830)`}>
          <text x={0} y={0} fontSize={130} fill={TEXT_PRI} textAnchor="middle" fontFamily="'Be Vietnam Pro', sans-serif" fontWeight={900} letterSpacing="6">
            KỸ NĂNG
          </text>
        </g>

        <g transform={`translate(${W / 2}, 980)`}>
          <text x={0} y={0} fontSize={84} fill={GOLD} textAnchor="middle" fontFamily="'Be Vietnam Pro', sans-serif" fontWeight={900} letterSpacing="3">
            AI KHÔNG thay
          </text>
        </g>
        ...
        <g transform={`translate(${W / 2}, 1110)`}>
          <text x={0} y={0} fontSize={42} fill={TEXT_PRI} textAnchor="middle" fontFamily="'Be Vietnam Pro', sans-serif" fontWeight={700} fontStyle="italic">
            Edge dài hạn · build từ giờ
          </text>
        </g>

        <g transform={`translate(${W / 2}, 1340)`}>
          {[
            { lbl: "taste", color: ACCENT_PINK },
            { lbl: "empathy", color: ACCENT_VIOLET },
            { lbl: "negotiate", color: ORANGE },
            { lbl: "original", color: GOLD },
            { lbl: "lead", color: ACCENT_BLUE },
          ].map((c, i) => (
