...
import beatsData from "./google_io_2026_beats.json";
...
export const GoogleIO2026: React.FC = () => {
  return (
    <AbsoluteFill style={{ background: "#06070D" }}>
      <Audio src={staticFile("google_io_2026/voice.mp3")} />
      {beats.map((b) => (
        <Sequence key={b.index} from={Math.round(b.start * FPS)} durationInFrames={Math.round(b.duration * FPS)}>
          <SlideBg src={`google_io_2026/slide_${b.index}.png`} duration={b.duration} />
        </Sequence>
      ))}
    </AbsoluteFill>
  );
