...
import beatsData from "./top10_skill_lost_beats.json";
...
export const Top10SkillLost: React.FC = () => {
  return (
    <AbsoluteFill style={{ background: "#08080F" }}>
      <Audio src={staticFile("top10_skill_lost/voice.mp3")} />
      {beats.map((b) => (
        <Sequence key={b.index} from={Math.round(b.start * FPS)} durationInFrames={Math.round(b.duration * FPS)}>
          <SlideBg src={`top10_skill_lost/slide_${b.index}.png`} duration={b.duration} />
        </Sequence>
      ))}
    </AbsoluteFill>
  );
