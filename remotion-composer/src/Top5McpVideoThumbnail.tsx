...
export const Top5McpVideoThumbnail: React.FC = () => (
  <AbsoluteFill>
    <svg width={W} height={H} style={{ position: "absolute", inset: 0 }}>
      ...
      <g transform={`translate(${W/2}, 820)`}>
        <text x={0} y={0} fontSize={120} fill={TEXT_PRI} textAnchor="middle" fontFamily="'Be Vietnam Pro', sans-serif" fontWeight={800} letterSpacing="0">
          MCP MUST-HAVE
        </text>
      </g>
      ...
      {/* MCP star badges grid */}
      <g transform="translate(0, 1080)">
        {[
          { label: "GitHub", star: "29.8k ⭐", color: ACCENT_BLUE, x: 80 },
          { label: "Context7", star: "55.2k ⭐", color: "#3DD9D6", x: 380 },
          { label: "Playwright", star: "32.4k ⭐", color: ACCENT_VIOLET, x: 680 },
        ].map((b, i) => (
          <g key={i} transform={`translate(${b.x}, 0)`}>
            <rect x={0} y={0} width={300} height={160} rx={16} fill={BG_SURFACE} stroke={b.color} strokeWidth={3} />
            <text x={150} y={64} fontSize={32} fill={TEXT_PRI} textAnchor="middle" fontFamily="'Be Vietnam Pro', sans-serif" fontWeight={800}>{b.label}</text>
            <text x={150} y={115} fontSize={32} fill={b.color} textAnchor="middle" fontFamily="'JetBrains Mono', monospace" fontWeight={700}>{b.star}</text>
          </g>
        ))}
      </g>
      ...
      <g transform="translate(0, 1280)">
        {[
          { label: "MarkItDown", star: "120k ⭐", color: GOLD, x: 100, big: true },
          { label: "Filesystem", star: "85.6k ⭐", color: GREEN, x: 520 },
        ].map((b, i) => (
          <g key={i} transform={`translate(${b.x}, 0)`}>
            <rect x={0} y={0} width={b.big ? 400 : 460} height={160} rx={16} fill={BG_SURFACE} stroke={b.color} strokeWidth={3} />
            <text x={(b.big ? 200 : 230)} y={64} fontSize={36} fill={TEXT_PRI} textAnchor="middle" fontFamily="'Be Vietnam Pro', sans-serif" fontWeight={800}>{b.label}</text>
            <text x={(b.big ? 200 : 230)} y={120} fontSize={42} fill={b.color} textAnchor="middle" fontFamily="'JetBrains Mono', monospace" fontWeight={700}>{b.star}</text>
          </g>
        ))}
      </g>
