...
import beatsData from "./sam_altman_truyen_ky_beats.json";
...
export const SamAltmanTruyenKy: React.FC = () => {
  return (
    <AbsoluteFill style={{ background: "#0A0612" }}>
      <Audio src={staticFile("sam_altman_truyen_ky/voice.mp3")} />
      {beats.map((b) => (
        <Sequence key={b.index} from={Math.round(b.start * FPS)} durationInFrames={Math.round(b.duration * FPS)}>
          <SlideBg src={`sam_altman_truyen_ky/slide_${b.index}.png`} duration={b.duration} />
        </Sequence>
      ))}
    </AbsoluteFill>
  );
