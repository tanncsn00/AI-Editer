...
export const Top10AgentPatternsThumbnail: React.FC = () => {
  return (
    <AbsoluteFill style={{ background: BG_DEEP }}>
      <svg width={W} height={H} style={{ position: "absolute", inset: 0 }}>
        ...
        <g transform={`translate(${W / 2}, 230)`}>
          <rect x={-330} y={-44} width={660} height={88} rx={44} fill={BG_SURFACE} stroke={ACCENT_VIOLET} strokeWidth={3} />
        ...
        <g transform={`translate(${W / 2}, 830)`}>
          <text x={0} y={0} fontSize={128} fill={TEXT_PRI} textAnchor="middle" fontFamily="'Be Vietnam Pro', sans-serif" fontWeight={900} letterSpacing="6">
            PATTERN
          </text>
        </g>
        ...
        <g transform={`translate(${W / 2}, 980)`}>
          <text x={0} y={0} fontSize={66} fill={GOLD} textAnchor="middle" fontFamily="'Be Vietnam Pro', sans-serif" fontWeight={900} letterSpacing="3">
            BUILD AGENT 2026
          </text>
        </g>
        ...
        <g transform={`translate(${W / 2}, 1110)`}>
          <text x={0} y={0} fontSize={40} fill={TEXT_PRI} textAnchor="middle" fontFamily="'Be Vietnam Pro', sans-serif" fontWeight={700} fontStyle="italic">
            xương sống · production-grade · Anthropic
          </text>
        </g>
        ...
        <g transform={`translate(${W / 2}, 1340)`}>
          {[
            { lbl: "chain", color: ACCENT_BLUE },
            { lbl: "route", color: ORANGE },
            { lbl: "parallel", color: GREEN },
            { lbl: "ReAct", color: ACCENT_CYAN },
            { lbl: "Reflexion", color: RED },
          ].map((c, i) => (
