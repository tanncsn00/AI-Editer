/**
 * IsseiDance3DSmpl — 3D Mixamo Soldier driven by GVHMR SMPL motion (proper SOTA pipeline).
 */

import { AbsoluteFill, Audio, staticFile, useCurrentFrame } from "remotion";
import { ThreeCanvas } from "@remotion/three";
import { GlbSmplDriven } from "./fx/GlbSmplDriven";
import motion from "./issei_smpl_motion.json";
  ...
  return (
    <svg width={W} height={H} style={{ position: "absolute", inset: 0 }}>
      <defs>
        ...
        <radialGradient id="moonglow4" cx="50%" cy="6%" r="22%">
          <stop offset="0%" stopColor="#FFE9B0" stopOpacity="0.55" />
          <stop offset="100%" stopColor="#FFE9B0" stopOpacity="0" />
        </radialGradient>
      </defs>
      <rect width={W} height={H * 0.78} fill="url(#nightsky4)" />
      <circle cx={W / 2} cy={H * 0.05} r={18} fill="#FFE9B0" opacity={lampPulse} />
      <ellipse cx={W / 2} cy={H * 0.10} rx={W * 0.45} ry={H * 0.35} fill="url(#moonglow4)" opacity={lampPulse} />
...
export const IsseiDance3DSmpl: React.FC = () => {
  return (
