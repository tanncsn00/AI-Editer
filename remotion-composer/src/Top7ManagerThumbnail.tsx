...
export const Top7ManagerThumbnail: React.FC = () => {
  return (
    <AbsoluteFill style={{ background: BG_DEEP }}>
      <svg width={W} height={H} style={{ position: "absolute", inset: 0 }}>
        <defs>
          <radialGradient id="mgt1" cx="20%" cy="0%" r="70%">
            <stop offset="0%" stopColor={VIOLET} stopOpacity="0.32" />
            <stop offset="100%" stopColor={BG_DEEP} stopOpacity="0" />
          </radialGradient>
          <radialGradient id="mgt2" cx="90%" cy="100%" r="60%">
            <stop offset="0%" stopColor={DRAGON_RED} stopOpacity="0.28" />
            <stop offset="100%" stopColor={BG_DEEP} stopOpacity="0" />
          </radialGradient>
          <radialGradient id="mgt3" cx="50%" cy="40%" r="50%">
            <stop offset="0%" stopColor={GOLD} stopOpacity="0.18" />
            <stop offset="100%" stopColor={BG_DEEP} stopOpacity="0" />
          </radialGradient>
          <pattern id="mgtgrid" width="48" height="48" patternUnits="userSpaceOnUse">
            <path d="M 48 0 L 0 0 0 48" fill="none" stroke={GOLD} strokeWidth="1" opacity="0.08" />
          </pattern>
          <radialGradient id="mgtmaskg" cx="50%" cy="50%" r="70%">
            <stop offset="20%" stopColor="white" stopOpacity="1" />
            <stop offset="100%" stopColor="white" stopOpacity="0" />
          </radialGradient>
          <mask id="mgtgm"><rect width={W} height={H} fill="url(#mgtmaskg)" /></mask>
          <filter id="mgtglow">
        ...
        <rect width={W} height={H} fill="url(#mgtgrid)" mask="url(#mgtgm)" />
        <rect width={W} height={H} fill="url(#mgt1)" />
        <rect width={W} height={H} fill="url(#mgt2)" />
        <rect width={W} height={H} fill="url(#mgt3)" />
        ...
        <g transform={`translate(${W / 2}, 340)`}>
          ...
          <text x={0} y={140} fontSize={120} fill={GOLD} textAnchor="middle" fontFamily="'Be Vietnam Pro', sans-serif" fontWeight={900} letterSpacing="4" filter="url(#mgtglow)">
            MANAGER
          </text>
          <text x={0} y={250} fontSize={52} fill={DRAGON_RED} textAnchor="middle" fontFamily="'Be Vietnam Pro', sans-serif" fontWeight={900} letterSpacing="2">
            KHIẾN DEV NHẬP MA
          </text>
        </g>
        ...
        <g transform={`translate(${W / 2}, 920)`}>
          {[
            { i: 0, lv: "LOẠI 1", n: "Thiên cơ quản lý", c: C_L1 },
            { i: 1, lv: "LOẠI 2", n: "Vô Thượng Họp Đạo", c: C_L2 },
            { i: 2, lv: "LOẠI 3", n: "Đại Năng Vi Mô", c: C_L3 },
            { i: 3, lv: "LOẠI 4", n: "Không biết tech", c: C_L4 },
            { i: 4, lv: "LOẠI 5", n: "Đại Đạo KPI", c: C_L5 },
            { i: 5, lv: "LOẠI 6", n: "Ẩn Thế Biến Mất", c: C_L6 },
            { i: 6, lv: "LOẠI 7", n: "Phật Tu Tích Cực", c: C_L7 },
          ].map((r) => (
        ...
        <g transform={`translate(${W / 2}, 1640)`}>
          ...
          <text x={0} y={-18} fontSize={34} fill={GOLD} textAnchor="middle" fontFamily="'Be Vietnam Pro', sans-serif" fontWeight={900}>
            SẾP BẠN LÀ LOẠI NÀO?
          </text>
          <text x={0} y={24} fontSize={22} fill={DRAGON_RED} textAnchor="middle" fontFamily="'Be Vietnam Pro', sans-serif" fontWeight={700}>
            Tag sếp · comment LOẠI · cười cùng đồng nghiệp
          </text>
        </g>
