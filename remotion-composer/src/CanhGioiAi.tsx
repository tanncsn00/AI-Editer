...
import beatsData from "./canh_gioi_ai_beats.json";
...
export const CanhGioiAi: React.FC = () => {
  return (
    <AbsoluteFill style={{ background: "#0A0612" }}>
      <Audio src={staticFile("canh_gioi_ai/voice.mp3")} />
      {beats.map((b) => (
        <Sequence key={b.index} from={Math.round(b.start * FPS)} durationInFrames={Math.round(b.duration * FPS)}>
          <SlideBg src={`canh_gioi_ai/slide_${b.index}.png`} duration={b.duration} />
        </Sequence>
      ))}
    </AbsoluteFill>
  );
