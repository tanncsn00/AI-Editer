...
import beatsData from "./gemini_spark_beats.json";
...
export const GeminiSpark: React.FC = () => {
  return (
    <AbsoluteFill style={{ background: "#06070D" }}>
      <Audio src={staticFile("gemini_spark/voice.mp3")} />
      {beats.map((b) => (
        <Sequence key={b.index} from={Math.round(b.start * FPS)} durationInFrames={Math.round(b.duration * FPS)}>
          <SlideBg src={`gemini_spark/slide_${b.index}.png`} duration={b.duration} />
        </Sequence>
      ))}
    </AbsoluteFill>
  );
