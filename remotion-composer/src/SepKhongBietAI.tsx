...
import beatsData from "./sep_khong_biet_ai_beats.json";
...
export const SepKhongBietAI: React.FC = () => {
  return (
    <AbsoluteFill style={{ background: "#08070D" }}>
      <Audio src={staticFile("sep_khong_biet_ai/voice.mp3")} />
      {beats.map((b) => (
        <Sequence key={b.index} from={Math.round(b.start * FPS)} durationInFrames={Math.round(b.duration * FPS)}>
          <SlideBg src={`sep_khong_biet_ai/slide_${b.index}.png`} duration={b.duration} />
        </Sequence>
      ))}
    </AbsoluteFill>
  );
