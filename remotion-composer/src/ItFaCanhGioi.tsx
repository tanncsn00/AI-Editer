...
import beatsData from "./it_fa_canh_gioi_beats.json";
...
export const ItFaCanhGioi: React.FC = () => {
  return (
    <AbsoluteFill style={{ background: "#0A0612" }}>
      <Audio src={staticFile("it_fa_canh_gioi/voice.mp3")} />
      {beats.map((b) => (
        <Sequence key={b.index} from={Math.round(b.start * FPS)} durationInFrames={Math.round(b.duration * FPS)}>
          <SlideBg src={`it_fa_canh_gioi/slide_${b.index}.png`} duration={b.duration} />
        </Sequence>
      ))}
    </AbsoluteFill>
  );
