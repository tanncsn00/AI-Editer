...
import beatsData from "./top5_mcp_auto_beats.json";
...
export const Top5McpAutomation: React.FC = () => {
  return (
    <AbsoluteFill style={{ background: "#08080F" }}>
      <Audio src={staticFile("top5_mcp_auto/voice.mp3")} />
      {beats.map((b) => (
        <Sequence key={b.index} from={Math.round(b.start * FPS)} durationInFrames={Math.round(b.duration * FPS)}>
          <SlideBg src={`top5_mcp_auto/slide_${b.index}.png`} duration={b.duration} />
        </Sequence>
      ))}
    </AbsoluteFill>
  );
