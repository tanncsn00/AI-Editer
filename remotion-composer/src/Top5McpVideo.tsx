...
import beatsData from "./mcp_video_beats.json";
import wordsData from "./mcp_video_words.json";
...
export const Top5McpVideo: React.FC = () => {
  return (
    <AbsoluteFill style={{ background: "#08080F" }}>
      <Audio src={staticFile("mcp_video/voice.mp3")} />
      {beats.map((b) => (
        <Sequence key={b.index} from={Math.round(b.start * FPS)} durationInFrames={Math.round(b.duration * FPS)}>
          <SlideBg src={`mcp_video/slide_${b.index}.png`} duration={b.duration} />
        </Sequence>
      ))}
      {/* Caption removed — slides have text content already, overlay = visual noise */}
    </AbsoluteFill>
  );
};
