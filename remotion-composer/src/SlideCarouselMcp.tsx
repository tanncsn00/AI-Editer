...
const SLIDES: Slide[] = [
  {
    ...
    hookHeadline: "5 MCP MUST-HAVE",
    hookSub: "cho Claude Code 2026",
    note: "Bỏ qua = mất 50% sức mạnh\ndata verified · star chính thức",
    color: ACCENT_SOLID,
  },
  {
    kind: "mcp", num: "2", total: "9",
    icon: "🐙", title: "GITHUB MCP",
    stars: "29.8k ⭐", badge: "Official · by GitHub",
    desc: ["Manage repos · PRs · issues", "Code search · CI/CD", "KHÔNG cần rời terminal"],
    cmd: "claude mcp add github \\\n  --transport http \\\n  --url https://api.githubcopilot.com/mcp/",
    note: "OAuth sẵn · không cần PAT",
    color: ACCENT_BLUE,
  },
  {
    kind: "mcp", num: "3", total: "9",
    icon: "📚", title: "CONTEXT7",
    stars: "55.2k ⭐", badge: "by Upstash",
    desc: ["Real-time docs version-specific", "Hết hallucinate API cũ", "Inject Next.js / React / Remotion"],
    cmd: "claude mcp add context7 --scope user \\\n  -- npx -y @upstash/context7-mcp",
    note: "Solve #1 pain · code outdated",
    color: ACCENT_CYAN,
  },
  {
    kind: "mcp", num: "4", total: "9",
    icon: "🎭", title: "PLAYWRIGHT MCP",
    stars: "32.4k ⭐", badge: "by Microsoft",
    desc: ["Browser automation accessibility", "E2E test · scrape · click form", "Faster than screenshot agents"],
    cmd: "claude mcp add playwright \\\n  -- npx @playwright/mcp@latest",
    note: "Test web · scrape data · regression",
    color: ACCENT_VIOLET,
  },
  {
    kind: "mcp", num: "5", total: "9",
    icon: "📄", title: "MARKITDOWN",
    stars: "120k ⭐", badge: "by Microsoft · KHỦNG NHẤT LIST",
    desc: ["Convert PDF / DOCX / XLSX / PPTX", "+ audio / image → MARKDOWN", "Drop brief PDF, slide PPT vào"],
    cmd: "claude mcp add markitdown \\\n  -- npx -y markitdown-mcp",
    note: "Claude đọc mọi format tự động",
    color: GOLD,
  },
  {
    kind: "mcp", num: "6", total: "9",
    icon: "📁", title: "FILESYSTEM",
    stars: "85.6k ⭐", badge: "Official · by Anthropic",
    desc: ["Sandboxed file ops vượt cwd", "Access control safe", "Multi-project workspace"],
    cmd: "claude mcp add filesystem -- \\\n  npx -y @modelcontextprotocol/\\\n  server-filesystem /allowed/path",
    note: "Cho 10+ project tại E:\\tvk",
    color: GREEN,
  },
  {
    kind: "bonus", num: "7", total: "9",
    icon: "🚀", title: "SUPABASE MCP",
    stars: "2.7k ⭐", badge: "BONUS · Supabase community",
    desc: ["Run SQL · manage tables", "Deploy edge functions", "Stack phổ biến VN startup"],
    cmd: "claude mcp add supabase --transport http \\\n  --url https://mcp.supabase.com/mcp",
    color: ACCENT_PINK,
  },
  {
    kind: "bonus", num: "8", total: "9",
    icon: "🧠", title: "SEQUENTIAL THINKING",
    stars: "Official", badge: "BONUS · zero-config",
    desc: ["Structured chain-of-thought", "Boost reasoning task khó", "Debug · refactor · architecture"],
    cmd: "claude mcp add sequential-thinking -- \\\n  npx -y @modelcontextprotocol/\\\n  server-sequential-thinking",
    color: ACCENT_VIOLET,
  },
  {
    kind: "cta", num: "9", total: "9",
    ctaTitle: "ĐÃ XÀI MCP NÀO?",
    ctaLines: ["💾  Save bài này", "💬  Comment MCP bạn xài", "🔗  Follow tip Claude Code daily"],
    note: "Data verified · 2026-05-13",
    color: ACCENT_SOLID,
  },
];
...
const HookSlide: React.FC<{ s: Slide }> = ({ s }) => (
  <g>
    ...
    <g transform={`translate(${W/2}, 700)`}>
      ...
      <text x={0} y={130} fontSize={108} fill={TEXT_PRI} textAnchor="middle" fontFamily="'Be Vietnam Pro', sans-serif" fontWeight={800} letterSpacing="0">
        MCP MUST-HAVE
      </text>
    </g>
...
const McpSlide: React.FC<{ s: Slide }> = ({ s }) => (
  <g>
    ...
    <g transform={`translate(${W/2}, 660)`}>
      <text x={0} y={0} fontSize={88} fill={TEXT_PRI} textAnchor="middle" fontFamily="'Be Vietnam Pro', sans-serif" fontWeight={800} letterSpacing="1">
        {s.title}
      </text>
    </g>
    {s.desc && (
...
export const SlideCarouselMcp: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ background: BG_DEEP }}>
      <BG />
      <svg width={W} height={H} style={{ position: "absolute", inset: 0 }}>
        {s.num && s.total && <NumberBadge num={s.num} total={s.total} color={s.color || ACCENT_SOLID} />}
