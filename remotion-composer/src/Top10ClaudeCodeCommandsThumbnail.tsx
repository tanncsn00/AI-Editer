...
export const Top10ClaudeCodeCommandsThumbnail: React.FC = () => {
  return (
    <AbsoluteFill style={{ background: BG_DEEP }}>
      <svg width={W} height={H} style={{ position: "absolute", inset: 0 }}>
        <defs>
          <radialGradient id="t1" cx="20%" cy="0%" r="70%">
            <stop offset="0%" stopColor={ACCENT_BLUE} stopOpacity="0.25" />
            <stop offset="100%" stopColor={BG_DEEP} stopOpacity="0" />
          </radialGradient>
          <radialGradient id="t2" cx="90%" cy="100%" r="60%">
            <stop offset="0%" stopColor={ACCENT_PINK} stopOpacity="0.22" />
            <stop offset="100%" stopColor={BG_DEEP} stopOpacity="0" />
          </radialGradient>
          <radialGradient id="t3" cx="50%" cy="40%" r="50%">
            <stop offset="0%" stopColor={ACCENT_VIOLET} stopOpacity="0.20" />
            <stop offset="100%" stopColor={BG_DEEP} stopOpacity="0" />
          </radialGradient>
        ...
        <g transform={`translate(${W / 2}, 950)`}>
          <text x={0} y={0} fontSize={140} fill={TEXT_PRI} textAnchor="middle" fontFamily="'Be Vietnam Pro', sans-serif" fontWeight={900} letterSpacing="6">
            LỆNH
          </text>
        </g>
        ...
        <g transform={`translate(${W / 2}, 1130)`}>
          <text x={0} y={0} fontSize={56} fill={TEXT_PRI} textAnchor="middle" fontFamily="'Be Vietnam Pro', sans-serif" fontWeight={700}>
            CLAUDE CODE
          </text>
          <text x={0} y={70} fontSize={42} fill={GOLD} textAnchor="middle" fontFamily="'Be Vietnam Pro', sans-serif" fontWeight={700} fontStyle="italic">
            phải biết — học 2 phút, xài cả đời
          </text>
        </g>
        ...
        <g transform={`translate(${W / 2}, 1380)`}>
          {[
            { lbl: "Shift+Tab", color: ACCENT_VIOLET },
            { lbl: "/clear", color: ACCENT_BLUE },
            { lbl: "@file", color: GOLD },
            { lbl: "!cmd", color: ACCENT_PINK },
            { lbl: "/cost", color: GOLD },
          ].map((c, i) => (
            <g key={i} transform={`translate(${(i - 2) * 184}, 0)`}>
              ...
              <text x={0} y={10} fontSize={24} fill={c.color} textAnchor="middle" fontFamily="'JetBrains Mono', monospace" fontWeight={700}>
                {c.lbl}
              </text>
            </g>
          ))}
        ...
        <g transform={`translate(${W / 2}, 1620)`}>
          <rect x={-460} y={-58} width={920} height={116} rx={20} fill={BG_SURFACE} stroke={ACCENT_PINK} strokeWidth={3} />
          <text x={0} y={-2} fontSize={32} fill={TEXT_PRI} textAnchor="middle" fontFamily="'Be Vietnam Pro', sans-serif" fontWeight={700}>
            Chồng kể vợ · không thuộc khỏi
          </text>
