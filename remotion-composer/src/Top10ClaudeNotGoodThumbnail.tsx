...
export const Top10ClaudeNotGoodThumbnail: React.FC = () => {
  return (
    <AbsoluteFill style={{ background: BG_DEEP }}>
      <svg width={W} height={H} style={{ position: "absolute", inset: 0 }}>
        ...
        <g transform={`translate(${W / 2}, 230)`}>
          <rect x={-340} y={-44} width={680} height={88} rx={44} fill={BG_SURFACE} stroke={RED} strokeWidth={3} />
          <text x={0} y={14} fontSize={32} fill={RED} textAnchor="middle" fontFamily="'Be Vietnam Pro', sans-serif" fontWeight={800} letterSpacing="3">
            ⚠️ SỰ THẬT · KHÔNG GÁY
          </text>
        </g>
        ...
        <g transform={`translate(${W / 2}, 980)`}>
          <text x={0} y={0} fontSize={62} fill={GOLD} textAnchor="middle" fontFamily="'Be Vietnam Pro', sans-serif" fontWeight={900} letterSpacing="2">
            Claude KHÔNG làm tốt
          </text>
        </g>
        ...
        {/* Limitation chips strip (warning style) */}
        <g transform={`translate(${W / 2}, 1340)`}>
          {[
            { lbl: "math", color: ORANGE },
            { lbl: "hallucinate", color: RED },
            { lbl: "xu nịnh", color: ACCENT_PINK },
            { lbl: "cost", color: GOLD },
            { lbl: "memory", color: ACCENT_BLUE },
          ].map((c, i) => (
