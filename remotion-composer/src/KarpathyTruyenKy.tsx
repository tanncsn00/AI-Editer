...
import beatsData from "./karpathy_truyen_ky_beats.json";
...
export const KarpathyTruyenKy: React.FC = () => {
  return (
    <AbsoluteFill style={{ background: "#0A0612" }}>
      <Audio src={staticFile("karpathy_truyen_ky/voice.mp3")} />
      {beats.map((b) => (
        <Sequence key={b.index} from={Math.round(b.start * FPS)} durationInFrames={Math.round(b.duration * FPS)}>
          <SlideBg src={`karpathy_truyen_ky/slide_${b.index}.png`} duration={b.duration} />
        </Sequence>
      ))}
    </AbsoluteFill>
  );
